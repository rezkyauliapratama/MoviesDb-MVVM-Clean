...
import id.co.rezkyauliapratama.rk_moviesdb_mvvm.common.authentication.SystemServices
import java.io.File
 ...
 */
internal class KeyStoreWrapper(private val context: Context, defaultKeyStoreName: String) {
    ...
    fun createAndroidKeyStoreAsymmetricKey(alias: String): KeyPair {
        val generator = KeyPairGenerator.getInstance(KeyProperties.KEY_ALGORITHM_RSA, KEYSTORE_TYPE)
    ...
    fun generateNewSymmetricPair(): Pair<ByteArray,ByteArray>{
        ...
        return Pair(key,ivSpec)
    }

    fun getSymmetricKey(keySpec: ByteArray, ivSpec: ByteArray): Pair<SecretKeySpec, AlgorithmParameterSpec> {
        val iv = IvParameterSpec(ivSpec)
        val skeySpec = SecretKeySpec(keySpec, "AES")

        return Pair(skeySpec,iv)
    }
}
